...
class LoginTest extends DuskTestCase
{
    ...
    public function test_funcionario_deveria_nao_logar_com_a_senha_errada()
    {
        $this->browse(function(Browser $browser) {
            $browser->visit('/')
                    ->clickLink('ENTRAR')
                    ...
                    ->click('[id=btn2]')
                    ->waitFor('@option2')
                    ->type('email', 'cradt@login')
                    ->type('password', 'senha errada')
                    ->click('[type=submit]')
                    ->assertSee('As informações de login não foram encontradas');
        });
    }
    public function test_funcionario_deveria_nao_logar_com_o_email_errado()
    {
        $this->browse(function(Browser $browser) {
            $browser->visit('/')
                    ->clickLink('ENTRAR')
    ...
    public function test_funcionario_deveria_logar()
    {
        $this->browse(function(Browser $browser) {
            $browser->visit('/')
                    ->clickLink('ENTRAR')
